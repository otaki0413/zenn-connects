...
Pages から Workers に移行するケースも多いと思うので、まず Git 統合を使用する前提で、両者の環境管理の違いを確認します。
...
ここまでで、Workers Builds の設定は一通り完了です。
ブランチへの push をトリガーにして、各 Worker へ自動デプロイが行われるようになりました。
