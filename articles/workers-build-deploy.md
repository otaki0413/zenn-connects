...
[`wrangler deploy`](https://developers.cloudflare.com/workers/wrangler/commands/#deploy) で 3 環境分を手動デプロイしておきます。
...
設定できるのは「本番ブランチ（1 つ）」と「非本番ブランチ（それ以外のすべて）」の 2 種類のみです。
そのため、たとえば `workers-builds`（develop 環境）の本番ブランチを `develop` に設定した場合、`staging` や `main` への push でも非本番ブランチのビルドが走ってしまうことを避けられません。特定のブランチだけをビルド対象にする、といった細かい制御は Workers Builds の設定だけでは実現できません。
