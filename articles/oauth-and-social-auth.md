...
特徴的なのは、**GitHub がリソースサーバーと認可サーバーの両方の役割を担っている**ことです。これにより、アプリは認可フローを通じて取得したアクセストークンで、GitHub API から直接ユーザー情報を取得できます。そのため、アプリ側で認証処理を挟まなくても、取得したユーザー情報（例：ユーザー名やメールアドレス）をもとにログイン（認証）できるのです。
